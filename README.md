...
Wildland fire is a multifaceted natural phenomenon of drastically increasing importance to both human and ecological communities that exist where wildland fires occur. This project combines two independent workflows that produce spatial datasets that can be used to understand cause and effect relationships between the datasets. Workflow A quantifies post-fire vegetation recovery using NEON hyperspectral reflectance data, whereas Workflow B categorizes burned areas by expected intensity using weather, topography and fuels data. In this project, we integrate these workflows for the [2016 Chimney Tops 2 Fire](https://www.nps.gov/grsm/learn/chimney-tops-2-fire.htm) by examining how the vegetation recovery (Workflow A) has progressed since the fire in areas of maxiumum versus minimum expected fire intensity (Workflow B). This project highlights how high spatial resolution (1-meter) remote sensing measurements, such as LiDAR topography and hyperspectral reflectance data, can be used to study fire recovery on the order of the spatial variation on the ground. In this repository, we include an [example notebook](https://github.com/AreteY/post-wildfire-recovery/tree/main/notebooks) that processes and analyzes hyperspectral reflectance data to assess the post-wildfire recovery with vegetation indices.
